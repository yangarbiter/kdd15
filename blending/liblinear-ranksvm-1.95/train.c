...
void read_problem(const char *filename)
{
	...
	while(readline(fp)!=NULL)
	{
		...
		while(1)
		{
			...
			if(p == NULL || *p == '\n') // check '\n' as ' ' may be after the last feature
				break;
			offset++;
			if(remove_index != -1 && offset == remove_index)
				continue;
			elements++;
		}
	...
	for(i=0;i<prob.l;i++)
	{
		...
		while(1)
		{
			...
			if(val == NULL)
				break;
			if (remove_index != -1 && atoi(idx) == remove_index)
			{
				continue;
			}
			else if (remove_index != -1 && atoi(idx) > remove_index)
			{	
				sprintf(shift_idx,"%d",atoi(idx)-1);
				idx = shift_idx;
			}
			if (strcmp(idx,"qid") == 0)
			{
